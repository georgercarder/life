...
class Universe {
  ...
  Universe (const int diameter) {
    ...
    for (int i = 0; i < diameter; ++i) {
      ...
      int threadRet = pthread_create(&threadId[i], 
                                     NULL, 
                                     initializeCurve, 
                                     (void*)(new initCurvePkg(diameter, essentialCurve)));
    } 
    for (int i = 0; i < diameter; ++i) {
    ...
    for (int i = 0; i < diameter; ++i) {
      int threadRet = pthread_create(&threadId[i], 
                                     NULL, 
                                     initializeCells, 
                                     (void*)(new initCellsPkg(i, i_space)));
    }
    for (int i = 0; i < diameter; ++i) {
      pthread_join(threadId[i], NULL); 
    }
    ...
    for (int i = 0; i < diameter; ++i) {
      ...
      int threadRet = pthread_create(&threadId[i], 
                                     NULL, 
                                     initializeLiving, 
                                     (void*)(new initLivingPkg(i, 
                                                               this, 
                                                               subLivingVectors.at(i))));
    }
    for (int i = 0; i < diameter; ++i) {
      pthread_join(threadId[i], NULL); 
    }
    ...
    long double ratio = (long double)i_activeVector->size() 
                        / (long double)(diameter * diameter);
  ...
  void moment() {
    ...
    pthread_t threadId[activeVector.size()];
    for (int i = 0; i < activeVector.size(); ++i) {
      // cell do thing 
      int threadRet = pthread_create(&threadId[i], 
                                     NULL, 
                                     Cell::testCellOperation, 
                                     NULL);
    } 
  ...
  static void* initializeCells(void* cellsPkg) {
    ...
    for (int i = 0; i < diameter; ++i) {
      for (int j = 0; j < 3; ++j) {
        for (int k = 0; k < 3; ++k) {
          std::array<int, 2> coord = {saferMod(i + rays.at(j), diameter), 
                                      saferMod(rowNum + rays.at(k), diameter)};
  ...
  static void* initializeLiving(void* living) {
    ...
    for (int i = 0; i < diameter; ++i) {
      ...
      if ((rand() % universe->i_densityCoefficient) == 0) { 
        ...
        std::vector<Cell*> nbrsNotIncluded = universe->i_space->at(rowNum)->at(i)
                                             ->getNeighborsNotInActiveVector();
